...
def smooth_boundaries(new_boundaries):
    ...
    smoothed_left = (
        sum(b[0] * w for b, w in zip(boundary_history, weights)) / total_weight
    )
    smoothed_right = (
        sum(b[1] * w for b, w in zip(boundary_history, weights)) / total_weight
    )
...
def draw_enhanced_visualization(
    ...
    # Calculate display positions for the tire region
    if flip_horizontal:
    ...
    if debug_mode:
        method_color = (
            (255, 255, 0) if "gradient" in detection_method else (0, 255, 255)
        )
    ...
    for i, section_name in enumerate(section_names):
        ...
        if temps.get("count", 0) > 0:
            ...
            temp_bg = pygame.Surface(
                (temp_label.get_width() + 20, temp_label.get_height() + 10)
            )
            temp_bg.fill((0, 0, 0))
            ...
            if debug_mode:
                ...
                if minmax_y + minmax_label.get_height() < y_offset + box_height - 100:
