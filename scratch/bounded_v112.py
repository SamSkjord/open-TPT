...
def analyze_tire_temperatures(middle_frame, threshold_offset):
    ...
    print(f"Detected tire boundaries: {tire_start} to {tire_end-1} (width: {tire_width})")
    print(f"Detection info: {{'tire_start': {tire_start}, 'tire_end': {tire_end}, 'tire_width': {tire_width}, 'detection_success': {detection_success}}}")
...
def draw_section_boundaries(screen, tire_stats, flip_horizontal, debug_mode, column_offset, x_offset, y_offset, box_height):
    colors = {
        "left": (255, 100, 100),
        "center": (100, 255, 100),
        "right": (100, 100, 255)
    ...
    for i, section_name in enumerate(section_names):
        ...
        screen.blit(box_surface, (box_start + x_offset, y_offset))

        rect = pygame.Rect(box_start + x_offset, y_offset, box_width, box_height)
        ...
        label_x = box_start + x_offset + (box_width - label.get_width()) / 2
        label_y = y_offset + 10
...
while True:
    ...
    # Aspect-ratio correct scaling: center the image, do not stretch
    img_aspect = img.width / img.height
    screen_aspect = DISPLAY_WIDTH / DISPLAY_HEIGHT

    if abs(img_aspect - screen_aspect) > 0.01:
        # Letterbox or pillarbox to maintain aspect
        ...
        scaled_surface = pygame.transform.scale(img_surface.convert(), (new_width, new_height))
        x_offset = (DISPLAY_WIDTH - new_width) // 2
        y_offset = (DISPLAY_HEIGHT - new_height) // 2
        box_height = new_height
    else:
        scaled_surface = pygame.transform.scale(img_surface.convert(), (DISPLAY_WIDTH, DISPLAY_HEIGHT))
        x_offset = 0
        y_offset = 0
        box_height = DISPLAY_HEIGHT

    screen.fill((0, 0, 0))  # fill black for letterbox bars
    screen.blit(scaled_surface, (x_offset, y_offset))

    draw_section_boundaries(
        screen, tire_stats,
        flip_horizontal, debug_mode, column_offset, x_offset, y_offset, box_height
    )
