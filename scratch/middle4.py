...
while True:
    ...
    # Show max temp from middle rows only
    max_temp = max(middle_frame)
    print("max temp (middle rows):", max_temp)
    label = font.render(f"{max_temp:.1f}C", 1, (255, 0, 0))
    screen.blit(label, (50, 50))
    
    # Optional: Show which rows are being displayed
    info_label = font.render(f"Rows {START_ROW}-{START_ROW + MIDDLE_ROWS - 1}", 1, (255, 255, 255))
    screen.blit(info_label, (50, 120))
