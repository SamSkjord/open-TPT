...
def analyze_tire_temperatures(middle_frame, threshold_offset):
    """Analyze tire temperature by robustly splitting the detected region into 3 thirds."""
    ...
    # --- Robust third boundaries ---
    thirds = [tire_width // 3] * 3
    for i in range(tire_width % 3):
        thirds[i] += 1
    ...
    for i, (start, end) in enumerate(boundaries):
        ...
        # Collect temperatures for each third using sensor indices
        for row in rows:
...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height, flip_horizontal, debug_mode, column_offset):
    """Draw bounding boxes for left/center/right thirds within the detected tire (hot) area ONLY."""
    ...
    tire_width = max(1, tire_end - tire_start)  # avoid zero division
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            ...
            # Clamp within tire_start and tire_end
...
while True:
    ...
    # Draw bounding boxes around detected thirds within tire
