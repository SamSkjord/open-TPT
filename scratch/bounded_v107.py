...
def analyze_tire_temperatures(middle_frame, threshold_offset):
    """Split hot region into 3 contiguous inclusive thirds, robust for any width."""
    ...
    # Robust split: always at least 1 column per section if possible
    if tire_width >= 3:
        base = tire_width // 3
        ...
        thirds = [1 if i < tire_width else 0 for i in range(3)]

    boundaries = []
    s = 0
    for w in thirds:
        boundaries.append((tire_start + s, tire_start + s + w))
        s += w
    ...
    for i, (start, end) in enumerate(boundaries):
        section_boundaries[section_names[i]] = {"start": start, "end": end}
        for row in rows:
            sections[section_names[i]].extend(row[start:end])
    ...
    # DEBUG: Print hot region and section splits for sanity check
    print(f"Detected tire/hot region: cols {tire_start} to {tire_end-1} (width={tire_width})")
    for sname in section_names:
        b = section_boundaries[sname]
        print(f"  {sname.upper()} [{b['start']}-{b['end']-1}]")
...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height, flip_horizontal, debug_mode, column_offset):
    """Draw bounding boxes strictly matching only the hot blob (tire region) thirds, with inclusive labels."""
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            bounds = tire_stats[section_name]["boundaries"]
            start_col = max(tire_start, min(SENSOR_WIDTH, bounds["start"] + column_offset))
            end_col   = max(tire_start, min(SENSOR_WIDTH, bounds["end"]   + column_offset))
            if start_col >= end_col:
                continue  # skip empty

            # Map pixel boundary: left edge of start_col to left edge of end_col
            if flip_horizontal:
                x_left  = ((SENSOR_WIDTH - end_col)   / SENSOR_WIDTH) * DISPLAY_WIDTH
                x_right = ((SENSOR_WIDTH - start_col) / SENSOR_WIDTH) * DISPLAY_WIDTH
            else:
                x_left  = (start_col / SENSOR_WIDTH) * DISPLAY_WIDTH
                x_right = (end_col   / SENSOR_WIDTH) * DISPLAY_WIDTH

            rect_x = min(x_left, x_right)
            rect_w = abs(x_right - x_left)

            # Draw semi-transparent fill
            box_surface = pygame.Surface((rect_w, DISPLAY_HEIGHT), pygame.SRCALPHA)
            box_surface.set_alpha(50)
            box_surface.fill(color)
            screen.blit(box_surface, (rect_x, 0))

            # Draw border
            rect = pygame.Rect(rect_x, 0, rect_w, DISPLAY_HEIGHT)
            pygame.draw.rect(screen, color, rect, 3)

            # Inclusive label: start to end-1
            small_font = pygame.font.Font(None, 40)
            label_text = f"{section_name.upper()} [{start_col}-{end_col-1}]"
            label = small_font.render(label_text, 1, color)
            label_x = rect_x + (rect_w - label.get_width()) / 2
            label_y = 10
