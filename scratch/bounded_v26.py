...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height):
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            ...
            box_surface.set_alpha(50)  # Semi-transparent
...
while True:
    ...
    if detection_info["detection_success"] and debug_mode:
        ...
        # Draw dots on hot pixels
        for row in range(MIDDLE_ROWS):
            for col in range(SENSOR_WIDTH):
                temp = middle_frame[row * SENSOR_WIDTH + col]
                if temp > threshold_temp:
                    # Map pixel to screen coordinates
                    x = (col / SENSOR_WIDTH) * DISPLAY_WIDTH + (DISPLAY_WIDTH / SENSOR_WIDTH) / 2
                    y = (row / MIDDLE_ROWS) * DISPLAY_HEIGHT + (DISPLAY_HEIGHT / MIDDLE_ROWS) / 2
                    
                    # Draw a small circle at hot pixel location
    ...
    instructions = "D: Debug | UP/DOWN: Adjust threshold | ESC: Exit"
