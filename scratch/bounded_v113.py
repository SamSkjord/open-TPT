...
def draw_section_boundaries(screen, tire_stats, flip_horizontal, debug_mode, column_offset, x_offset, y_offset, box_width, box_height):
    colors = {
        "left": (255, 100, 100),
        "center": (100, 255, 100),
        "right": (100, 100, 255)
    ...
    # Use only the scaled "active" region for overlays
    if flip_horizontal:
    ...
    print(f"Overlay region: {region_left:.2f} to {region_right:.2f} (active width: {box_width}) in window {x_offset}–{x_offset+box_width}")
...
while True:
    ...
    # Aspect-ratio correct scaling: center the image, do not stretch/crop sensor columns!
    ...
    draw_section_boundaries(
        screen, tire_stats,
        flip_horizontal, debug_mode, column_offset, x_offset, y_offset, box_width, box_height
    )
