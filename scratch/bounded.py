...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height):
    ...
    # Calculate the position and size of the thermal image on screen
    # The image is scaled to fill the screen
    x_scale = DISPLAY_WIDTH / interpolated_width
    y_scale = DISPLAY_HEIGHT / interpolated_height
    
    # Since the image fills the screen, we use the same scale for both dimensions
    scale = min(x_scale, y_scale)
    
    # Calculate actual displayed image dimensions
    displayed_width = interpolated_width * scale
    displayed_height = interpolated_height * scale
    
    # Calculate offset to center the image
    x_offset = (DISPLAY_WIDTH - displayed_width) / 2
    y_offset = (DISPLAY_HEIGHT - displayed_height) / 2
    
    # For fullscreen, the image fills the entire screen
    if scale == x_scale:
        displayed_width = DISPLAY_WIDTH
        x_offset = 0
    else:
        displayed_height = DISPLAY_HEIGHT
        y_offset = 0
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            ...
            # Calculate pixel positions in the displayed image
            # Map from sensor coordinates to display coordinates
            x_start = (boundaries["start"] * INTERPOLATE * displayed_width) / interpolated_width + x_offset
            x_end = (boundaries["end"] * INTERPOLATE * displayed_width) / interpolated_width + x_offset
            
            # Y coordinates cover all middle rows
            y_start = y_offset
            y_end = displayed_height + y_offset
            
            # Draw rectangle
            rect_width = x_end - x_start
            rect = pygame.Rect(x_start, y_start, rect_width, displayed_height)
...
while True:
    ...
    print(
        f"Tire detected: {tire_start}-{tire_end-1} (W:{tire_width}) - Left: {tire_stats['left']['avg']:.1f}°C, Center: {tire_stats['center']['avg']:.1f}°C, Right: {tire_stats['right']['avg']:.1f}°C"
    )
