elif event.key == pygame.K_v:  # Press 'V' to verify coordinates
                print("\nCoordinate Verification:")
                print(f"Display: {DISPLAY_WIDTH}x{DISPLAY_HEIGHT}")
                print(f"Sensor: {SENSOR_WIDTH}x{MIDDLE_ROWS}")
                print(f"Interpolated image: {SENSOR_WIDTH * INTERPOLATE}x{MIDDLE_ROWS * INTERPOLATE}")
                print(f"Each sensor column should be {DISPLAY_WIDTH/SENSOR_WIDTH:.1f} pixels wide")
                print("\nColumn → Screen X mapping:")
                for col in [0, 10, 16, 22, 29, 31]:
                    x = (col / SENSOR_WIDTH) * DISPLAY_WIDTH
                    print(f"  Column {col} → X: {x:.1f}")
                print()        # Test: Draw a box at columns 22-29 to verify mapping
        test_start = 22
        test_end = 30  # exclusive
        test_x_start = (test_start / SENSOR_WIDTH) * DISPLAY_WIDTH
        test_x_end = (test_end / SENSOR_WIDTH) * DISPLAY_WIDTH
        
        test_surface = pygame.Surface((test_x_end - test_x_start, 30))
        test_surface.set_alpha(100)
        test_surface.fill((255, 255, 255))
        screen.blit(test_surface, (test_x_start, DISPLAY_HEIGHT - 200))
        
        test_label = ref_font.render(f"Test: Cols 22-29", 1, (255, 255, 255))
        screen.blit(test_label, (test_x_start + 5, DISPLAY_HEIGHT - 195))
        
        # Print calculation for debugging
        calc_text = f"Col 22-29: X={test_x_start:.0f}-{test_x_end:.0f} (Width={test_x_end-test_x_start:.0f}px)"
        calc_label = ref_font.render(calc_text, 1, (255, 255, 255))
        screen.blit(calc_label, (50, DISPLAY_HEIGHT - 250))            elif event.key == pygame.K_c:  # Press 'C' to toggle column test
                show_column_test = not show_column_test
                print(f"Column test: {'ON' if show_column_test else 'OFF'}")
                if show_column_test:
                    print("Test lines at columns: 0, 8, 16, 24, 31")
            elif event.key == pygame.K_v:  # Press 'V' to verify coordinates
                print("\nCoordinate Verification:")
                print(f"Display: {DISPLAY_WIDTH}x{DISPLAY_HEIGHT}")
                print(f"Sensor: {SENSOR_WIDTH}x{MIDDLE_ROWS}")
                ...
                for col in [0, 10, 16, 22, 29, 31]:
                    x = (col / SENSOR_WIDTH) * DISPLAY_WIDTH
                    print(f"  Column {col} → X: {x:.1f}")
                print()        # Show a reference box at column 16 (middle of sensor)
        ref_col = 16
        ref_x_start = (ref_col / SENSOR_WIDTH) * DISPLAY_WIDTH
        ref_x_end = ((ref_col + 1) / SENSOR_WIDTH) * DISPLAY_WIDTH
        ref_rect = pygame.Rect(ref_x_start, 0, ref_x_end - ref_x_start, 50)
        pygame.draw.rect(screen, (0, 255, 255), ref_rect, 2)
        
        ref_font = pygame.font.Font(None, 25)
        ref_label = ref_font.render("Col 16", 1, (0, 255, 255))
        screen.blit(ref_label, (ref_x_start + 5, 55))
        
        # Test: Draw a box at columns 22-29 to verify mapping
        test_start = 22
        test_end = 30  # exclusive
        test_x_start = (test_start / SENSOR_WIDTH) * DISPLAY_WIDTH
        test_x_end = (test_end / SENSOR_WIDTH) * DISPLAY_WIDTH
        ...
        calc_text = f"Col 22-29: X={test_x_start:.0f}-{test_x_end:.0f} (Width={test_x_end-test_x_start:.0f}px)"
        calc_label = ref_font.render(calc_text, 1, (255, 255, 255))
        screen.blit(calc_label, (50, DISPLAY_HEIGHT - 250))    # Show coordinate mapping info on screen (debug mode)
    if debug_mode:
        coord_y = 300
        ...
        # Show a reference box at column 16 (middle of sensor)
        ref_col = 16
        ref_x_start = (ref_col / SENSOR_WIDTH) * DISPLAY_WIDTH
        ref_x_end = ((ref_col + 1) / SENSOR_WIDTH) * DISPLAY_WIDTH
        ref_rect = pygame.Rect(ref_x_start, 0, ref_x_end - ref_x_start, 50)
        ...
        ref_font = pygame.font.Font(None, 25)
        ref_label = ref_font.render("Col 16", 1, (0, 255, 255))
        screen.blit(ref_label, (ref_x_start + 5, 55))            elif event.key == pygame.K_f:  # Press 'F' to flip horizontally
                flip_horizontal = not flip_horizontal
                print(f"Horizontal flip: {'ON' if flip_horizontal else 'OFF'}")import time
import math
import pygame
...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height, flip_horizontal, debug_mode):
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            ...
            # boundaries["start"] is inclusive, boundaries["end"] is exclusive
            if flip_horizontal:
                # When flipped, we need to mirror the coordinates
                x_start = ((SENSOR_WIDTH - boundaries["end"]) / SENSOR_WIDTH) * DISPLAY_WIDTH
                x_end = ((SENSOR_WIDTH - boundaries["start"]) / SENSOR_WIDTH) * DISPLAY_WIDTH
            else:
...
while True:
    ...
    if detection_info["detection_success"] and debug_mode:
        if flip_horizontal:
            tire_x_start = ((SENSOR_WIDTH - tire_end) / SENSOR_WIDTH) * DISPLAY_WIDTH
            tire_x_end = ((SENSOR_WIDTH - tire_start) / SENSOR_WIDTH) * DISPLAY_WIDTH
        else:
    ...
    instructions = "D: Debug | F: Flip | C: Columns | V: Verify | T: Test | UP/DOWN: Threshold"
    instructions_label = small_font.render(instructions, 1, (150, 150, 150))
    screen.blit(instructions_label, (DISPLAY_WIDTH - instructions_label.get_width() - 20, DISPLAY_HEIGHT - 20))
