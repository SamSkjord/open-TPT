...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height, flip_horizontal, debug_mode, column_offset):
    """Draw exactly 3 overlay boxes covering the detected hot region, each box is 1/3 of the region in display coordinates."""
    ...
    # Convert tire_start/tire_end (sensor columns) to screen coordinates
    if flip_horizontal:
        x_start = ((SENSOR_WIDTH - tire_end) / SENSOR_WIDTH) * DISPLAY_WIDTH
        x_end   = ((SENSOR_WIDTH - tire_start) / SENSOR_WIDTH) * DISPLAY_WIDTH
    else:
        x_start = (tire_start / SENSOR_WIDTH) * DISPLAY_WIDTH
        x_end   = (tire_end / SENSOR_WIDTH) * DISPLAY_WIDTH

    # Ensure left < right
    region_left = min(x_start, x_end)
    region_right = max(x_start, x_end)
    region_width = region_right - region_left

    section_names = ["left", "center", "right"]

    for i, section_name in enumerate(section_names):
        # Each section gets a third of the detected region
        seg_start = region_left + i * (region_width / 3)
        seg_end = region_left + (i + 1) * (region_width / 3)

        # Draw box
        box_surface = pygame.Surface((seg_end - seg_start, DISPLAY_HEIGHT), pygame.SRCALPHA)
        box_surface.set_alpha(50)
        box_surface.fill(colors[section_name])
        screen.blit(box_surface, (seg_start, 0))

        # Border
        rect = pygame.Rect(seg_start, 0, seg_end - seg_start, DISPLAY_HEIGHT)
        ...
        label_x = seg_start + ((seg_end - seg_start) - label.get_width()) / 2
        label_y = 10
