...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height, flip_horizontal, debug_mode, column_offset):
    """Draw bounding boxes around the detected tire sections, covering only the columns actually used."""
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            ...
            # Apply offset and clamp to legal sensor columns
            start_col = max(0, min(SENSOR_WIDTH, bounds["start"] + column_offset))
            end_col   = max(0, min(SENSOR_WIDTH, bounds["end"]   + column_offset))
            if flip_horizontal:
                x_start = ((SENSOR_WIDTH - end_col) / SENSOR_WIDTH) * DISPLAY_WIDTH
                x_end   = ((SENSOR_WIDTH - start_col) / SENSOR_WIDTH) * DISPLAY_WIDTH
            else:
                x_start = (start_col / SENSOR_WIDTH) * DISPLAY_WIDTH
                x_end   = (end_col   / SENSOR_WIDTH) * DISPLAY_WIDTH

            # Clamp and compute rectangle
            x_start = max(0, min(DISPLAY_WIDTH, x_start))
            x_end   = max(0, min(DISPLAY_WIDTH, x_end))
            rect_x = min(x_start, x_end)
            rect_w = abs(x_end - x_start)
            ...
            label_text = f"{section_name.upper()} [{bounds['start']}-{bounds['end']-1}]"
