...
def draw_section_boundaries(screen, tire_stats, interpolated_width, interpolated_height, flip_horizontal, debug_mode, column_offset):
    ...
    for section_name, color in colors.items():
        if section_name in tire_stats and "boundaries" in tire_stats[section_name]:
            ...
            # The bounds *should already be within tire_start:tire_end*, but let's clamp just in case
            ...
            # Map sensor column to display X within the context of the *whole image* (which is always stretched full width)
            # This is because the image is always mapped 0-31 => 0-DISPLAY_WIDTH, but only tire region is divided in thirds
...
while True:
    ...
    # --- you can add your other overlays and info draws here, as in your original ---
